package Net::OSCAR::Direct::Listener;
...
sub connect($$) {
	...
	if(defined($self->{session}->{proxy_type}) and $self->{session}->{proxy_type} =~ /^SOCKS(4|5)/) {
		...
		$self->set_blocking(0);
		(undef, undef, $self->{port}) = $sock->bind(peer_port => $self->{session}->{options}->{direct_connect_port}) or return $self->{session}->craout($self, "Couldn't SOCKS bind: $@");
	} else {
		$self->{socket} = gensym;
...
sub process_one($;$$$) {
	...
	if($read) {
		...
		my $connection = $self->{session}->addconn(conntype => OSCAR_CONNTYPE_OUT, description => "direct connection", state => "read", dcstate => Net::OSCAR::Connection::Direct::STATE_EXPECTING_HEADER, socket => $newsock);
